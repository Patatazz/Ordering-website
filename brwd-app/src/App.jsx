...
const App = () => {
  ...
  const submitReview = async (orderId, rating, comment) => {
    ...
    try {
        ...
        // Fallback if AI fails: still save the review without sentiment
        setSuccessModal('Review submitted successfully.');
        // (You would add fallback save logic here in a production app)
    }
  };
            ...
            <div className="px-8 py-12 max-w-2xl mx-auto">
                ...
                <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow-lg p-8 space-y-6">
                    ...
                    <input
                        ...
                        required
                        className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-amber-500 bg-gray-50"
                    />
                    </div>
                ...
                <div className="flex-1">
                  ...
                  <div className="flex flex-col gap-2">
                    <div className="flex items-center gap-4">
                        ...
                        {getAverageRating(selectedProduct)} ({getReviewCount(selectedProduct)} reviews)
                        </span>
                    </div>
            ...
            {filteredReviews.length > 0 ? (
              ...
              <div className="bg-white rounded-xl shadow-lg p-12 text-center">
                <p className="text-xl text-gray-600">
                    {filter === 'ALL' ? 'No reviews yet for this product' : `No ${filter.toLowerCase()} reviews found`}
                </p>
              </div>
            )}
          </main>
                  ...
                  >
                    ...
                    {reviewCount > 0 ? (
                      <div>
                        ...
                        <p className="text-sm text-gray-600 mt-2">{reviewCount} review{reviewCount !== 1 ? 's' : ''}</p>
                      </div>
                    ) : (
    ...
    const renderProductCard = (product) => (
        <div key={product.id} className="bg-white rounded-xl shadow-lg p-6 space-y-3 border-4 border-transparent hover:border-red-500 transition">
            <img 
              src={product.image} 
              alt={product.name} 
              className="block w-32 h-32 object-contain mx-auto mb-4 bg-gray-50 rounded-lg" 
            />
